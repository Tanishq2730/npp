...
export default async function resolvePreviewUrl(
  ...
  let baseUrl = `http://localhost:3000/en-US`;
  ...
  if (process.env.SANITY_STUDIO_VERCEL_ENV) {
    ...
    baseUrl =
      process.env.SANITY_STUDIO_VERCEL_ENV === "production"
        ? `https://demo-blog-platform.sanity.build`
        : // This should work, but doesn't
          ...
          `https://demo-blog-platform-git-${process.env.SANITY_STUDIO_VERCEL_GIT_COMMIT_REF}.sanity.build`;
  }

  const { _id } = doc;

  const previewUrl = new URL(`/pressRelease`, baseUrl);
